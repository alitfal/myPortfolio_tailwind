...
const Proyectos = () => {
  ...
  const translatedProyectos = useMemo(
    () =>
    ...
    <div className="flex flex-col flex-grow bg-gray-800 text-white p-4">
      <header className="container mx-auto text-center">
        <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-blue-500">
          {t("translation.titleProjects")}
        </h1>
        <input
          className="mt-4 mb-2 p-2 bg-gray-700 border border-gray-600 rounded w-full md:w-1/2 lg:w-1/3"
          type="text"
          value={searchTerm}
          ...
          <table className="w-full text-left rounded-lg overflow-hidden">
            <thead className="bg-gray-700">
              <tr>
                <th scope="col" className="p-4">
                  {t("translation.titleProject")}
                </th>
                <th scope="col" className="p-4">
                  {t("translation.titleDescription")}
                </th>
              </tr>
            </thead>
            <tbody className="bg-gray-600">
              {filteredProyectos.map((proyecto) => (
                <tr
                  key={proyecto.id}
                  className="border-b border-gray-700 hover:bg-gray-500 transition duration-300 ease-in-out"
                >
